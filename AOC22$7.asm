...
* R4 will have the total file size for the current dir
         SR    R3,R3
         AFI   R3,TOTAL_SPACE
* R3 will have the min size for a dir > need size
...
NEW_DIR  EQU *
         ...
         J     GET_LINE        Get next
BACK_DIR EQU *
         CLFI  R4,NEED_SPACE       Compare against needed space
         JL    GO_PACK1            Lower than we need, ignore
         CR    R3,R4               Compare against current dir
         JL    GO_PACK1            We had lower already, ignore
         LR    R3,R4               This is lower, save it
...
DEL3X    CVD   R3,CPACKED          Convert to decimal to display
...
FILE#EOD EQU   *
         ...
         JNH   DONE
         CLFI  R4,NEED_SPACE       Compare against needed space
         JL    GO_PACK2            Lower than we need, ignore
         CR    R3,R4               Compare against current dir
         JL    GO_PACK2            We had lower already, ignore
         LR    R3,R4               This is lower, save it
GO_PACK2 CVD   R4,CPACKED          Convert to decimal to display
         ...
         AHI   R10,-4          Go to the prev dir (fullword)
         A     R4,0(,R10)      Add subdir to current total
DEL3     CVD   R3,CPACKED          Convert to decimal to display
         UNPK  RECOUT+00(13),CPACKED(L'CPACKED+1)
         MVI   RECOUT+00+11,C' ' Get rid of the "C" for packed dec
         PUT   FILEOUT,RECOUT
         J     FILE#EOD
...
*
TOTAL_SPACE EQU 70000000
USED_SPACE  EQU 43598596   From first star, usage at root
FREE_SPACE  EQU TOTAL_SPACE-USED_SPACE
UPDAT_SPACE EQU 30000000
NEED_SPACE  EQU UPDAT_SPACE-FREE_SPACE
*
*URDIR   DC    X'01F0F0F0'
